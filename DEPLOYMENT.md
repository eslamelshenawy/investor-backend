...
الـ Scheduler موجود في `src/jobs/scheduler.ts` ويعمل **أوتوماتيك** في Production:
...
يعني على Render هيشتغل أوتوماتيك لأن `NODE_ENV=production`.

### جدول المهام

| Job | التوقيت | Cron Expression | الوظيفة |
|-----|---------|-----------------|---------|
| **Discovery** | كل يوم أحد الساعة 3 صباحاً | `0 3 * * 0` | اكتشاف Datasets جديدة |
| Full Sync | كل 6 ساعات | `0 */6 * * *` | مزامنة البيانات الحكومية |
| AI Analysis | كل 6 ساعات (offset 30 دقيقة) | `30 */6 * * *` | توليد إشارات ذكية |
| Content Gen | يومياً الساعة 6 صباحاً | `0 6 * * *` | توليد محتوى بالـ AI |
| Quick Check | كل ساعة | `0 * * * *` | تنظيف الـ Signals المنتهية |
| Cache Refresh | كل 30 دقيقة | `*/30 * * * *` | تحديث الـ Cache |

### التشغيل اليدوي عبر API

كمان تقدر تشغل أي Job يدوي من الـ API (يحتاج صلاحية Admin):

| Method | Endpoint | الوظيفة |
|--------|----------|---------|
| POST | `/api/discovery/discover` | اكتشاف فقط |
| POST | `/api/discovery/discover-and-sync` | اكتشاف + إضافة + مزامنة |
| POST | `/api/discovery/sync-all` | مزامنة كل الـ Datasets |
| POST | `/api/admin/trigger-sync` | تشغيل Full Sync يدوي |
| POST | `/api/admin/trigger-analysis` | تشغيل AI Analysis يدوي |
